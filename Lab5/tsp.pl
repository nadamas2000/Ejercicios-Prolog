...
tsp( [], AccumulatedKm, RouteSoFar ):-
    storeRouteIfBetter(AccumulatedKm,RouteSoFar), fail.  % No quedan ciudades por visitar: guardar solución
tsp( Cities, AccumulatedKm, [ CurrentCity | _ ] ):-
    bestRouteSoFar(Km,_),
    lowerBound(Cities, CurrentCity, LB),
    AccumulatedKm + LB >= Km, !, fail.
%tsp(  _, AccumulatedKm, _          ):-
%    bestRouteSoFar(Km,_), AccumulatedKm >= Km, !, fail.  % si Kms >= mejor solución existente: fail (poda)
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-                                  % Visitar siguiente.
...
% Minimum distance between CurrentCity and rest of cities.
lowerBound([City], CurrentCity, Km) :-
    distance(City, CurrentCity, Km), !.             % One [City] -> Km = distance between CurrentCity and City.
lowerBound([City|Cities], CurrentCity, Km) :-
    lowerBound(Cities, CurrentCity, KmMinRest),     % KmMinRest = Min distance between CurrentCity and other Cities.
    distance(City, CurrentCity, D),                 % D is distance between CurrentCity and City.
    (KmMinRest < D -> Km is KmMinRest ; Km is D).   % if (KmMinRest < D) then Km=KmMinRest else Km=D;
    

storeRouteIfBetter( Km, Route ):-  bestRouteSoFar( BestKm, _ ), Km < BestKm,                % Guarda ruta si es mejor
    write('Improved solution. New best distance is '),
    write(Km),
    write(' km.'), nl,
    retractall(bestRouteSoFar(_,_)),
    assertz(bestRouteSoFar(Km,Route)), !.
